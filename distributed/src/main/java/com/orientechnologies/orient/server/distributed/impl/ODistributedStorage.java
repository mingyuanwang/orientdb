...
public class ODistributedStorage implements OStorage, OFreezableStorageComponent, OAutoshardedStorage {
        ...
        for (Object r : resultSet) {
          if (r instanceof ODocument) {
            ...
            for (Map.Entry<String, Object> p : proj.entrySet()) {
              ...
              if (!(p.getValue() instanceof OSQLFunctionRuntime))
                doc.field(p.getKey(), (Object) ((ODocument) r).field(p.getKey()));
            }
          }
        }
      }
  ...
  @Override
  public void open(String iUserName, String iUserPassword, final OContextConfiguration contextConfiguration) {
  ...
  void executeUndoOnLocalServer(final ODistributedRequestId reqId, final OAbstractReplicatedTask task) {
    ...
    if (undoTask != null) {
      OScenarioThreadLocal.executeAsDistributed(new Callable<Object>() {
        @Override
        public Object call() throws Exception {
          ODatabaseDocumentInternal database = (ODatabaseDocumentTx) ODatabaseRecordThreadLocal.INSTANCE.getIfDefined();
          final boolean databaseAlreadyDefined;

          if (database == null) {
            databaseAlreadyDefined = false;
            database = serverInstance.openDatabase(getName(), null, null, null, true);
          } else
